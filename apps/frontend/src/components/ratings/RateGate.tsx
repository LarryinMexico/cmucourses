 ...
 */
const RateGate = ({ targetType, targetID }: { targetType: RatingTargetType; targetID: string }) => {
  ...
  if (!isSignedIn) {
    return (
      <div className="text-gray-400 text-sm">
        <SignInButton /> to rate this {targetType === "COURSE" ? "course" : "instructor"}.
      </div>
    );
  }
  ...
  if (targetType === "COURSE") {
    ...
    if (!taken) {
      return (
        <div className="text-gray-400 text-sm">
          Mark this course as Taken on your <Link href="/profile">profile</Link> to rate it.
        </div>
      );
    }
  }

  return <RatingForm targetType={targetType} targetID={targetID} existing={ownRating} />;
};
